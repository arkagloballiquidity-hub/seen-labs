...
export function LoginPage() {
  ...
  if (user) {
    if (role === 'admin') navigate('/crm', { replace: true })
    else navigate('/mi-proyecto', { replace: true })
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // onAuthStateChange will trigger role resolution; redirect after short delay
    setTimeout(() => {
      const stored = localStorage.getItem('sb-role')
      navigate(stored === 'admin' ? '/crm' : '/mi-proyecto', { replace: true })
    }, 400)
  }
    ...
    <div style={{ minHeight:'100vh', background:'#0A0A0F', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'sans-serif' }}>
      <div style={{ width:'100%', maxWidth:380, padding:'0 24px' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:40 }}>
          ...
          <p style={{ fontSize:12, color:'#6B6880', letterSpacing:'0.15em', textTransform:'uppercase' }}>Portal de acceso</p>
        </div>
